...
import {
  getAllTypeForCustomerQuery,
  getAllWishlistsForCustomerQuery,
...
export const baseGetAll = (
  ...
  return async (
    ...
    if (!isDefinedString(token) || !isDefinedString(customerId)) {
    ...
    try {
      const items = await getDatabase()
        .query(getAllWishlistsForCustomerQuery(customerId))
        .promise();
